...
Open.bouts <- ggplot(Arm.stats, aes(Geno, Open.Bouts.mean))+
        geom_bar(stat = "identity", width = 0.7)+
        scale_fill_grey(start = 0.25, end = 0.75)+
        geom_errorbar(aes(ymin = Open.Bouts.mean - Open.Bouts.sem, ymax = Open.Bouts.mean + Open.Bouts.sem), width = .4)+
        xlab(NULL)+
        ylab("Number of Bouts in Open Arms")

Open.dur <- ggplot(Arm.stats, aes(Geno, Open.Dur.mean))+
        geom_bar(stat = "identity", width = 0.7)+
        scale_fill_grey(start = 0.25, end = 0.75)+
        geom_errorbar(aes(ymin = Open.Dur.mean - Open.Dur.sem, ymax = Open.Dur.mean + Open.Dur.sem), width = .4)+
        xlab(NULL)+
        ylab("Duration in Open Arms (sec)")

Closed.bouts <- ggplot(Arm.stats, aes(Geno, Closed.Bouts.mean))+
        geom_bar(stat = "identity", width = 0.7)+
        scale_fill_grey(start = 0.25, end = 0.75)+
        geom_errorbar(aes(ymin = Closed.Bouts.mean - Closed.Bouts.sem, ymax = Closed.Bouts.mean + Closed.Bouts.sem), width = .4)+
        xlab(NULL)+
        ylab("Number of Bouts in Closed Arms")

Closed.dur <- ggplot(Arm.stats, aes(Geno, Closed.Dur.mean))+
        geom_bar(stat = "identity", width = 0.7)+
        scale_fill_grey(start = 0.25, end = 0.75)+
        geom_errorbar(aes(ymin = Closed.Dur.mean - Closed.Dur.sem, ymax = Closed.Dur.mean + Closed.Dur.sem), width = .4)+
        xlab(NULL)+
